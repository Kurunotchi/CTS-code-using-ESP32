...
class WiFiBatteryMonitor:
    ...
    def setup_gui(self):
        ...
        for s in "ABCD":
            ...
            lbl = ttk.Label(frame, text="No Batt# | --- V | --- mA | --- Ah | ---")
    ...
    def process_json(self, data):
        ...
        if data.get("type") == "sensor_data":
            ...
            self.live_labels[s].config(
                text=f"{batt_display} | {data['voltage']:.2f}V | {data['current']:.0f}mA | {self.slots[s]['simpson_capacity']:.6f}Ah | {mode}{cycle_display}"
            )
            ...
            if current_tab < 4:  # If a slot tab is selected
                ...
                if selected_slot == s:  # Only log if it matches the incoming data's slot
                    ...
                    self.log(
                        ...
                        f"Simpson Cap: {self.slots[s]['simpson_capacity']:.6f}Ah | "
    ...
    def compute_simpson_capacity(self, s):
        ...
        # Convert from mA*s to Ah (divide by 3600000)
        capacity_ah = integral / 3600000
        self.slots[s]["simpson_capacity"] = capacity_ah
        self.slots[s]["capacity"].append(capacity_ah)
    ...
    def update_stats_table(self):
        ...
        table = ax4.table(cellText=table_data,
                         colLabels=["Slot", "V", "I", "Cap(Ah)", "Mode", "Cycle", "Batt#", "Samples"],
                         cellLoc='center',
                         loc='center',
                         colColours=['#4472c4'] * 8)
